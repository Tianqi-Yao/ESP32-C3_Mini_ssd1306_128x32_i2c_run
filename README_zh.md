...
| ESP32-C3 引脚 | 信号 / 功能 | 方向 | 连接到 | 备注 |
|---|---|---|---|---|
| **GPIO21** | I2C SDA | 双向 | DS3231 SDA、SHT35 SDA | 共用 I2C 总线 |
| **GPIO20** | I2C SCL | 输出 | DS3231 SCL、SHT35 SCL | 共用 I2C 总线 |
| **GPIO10** | 电源锁存控制 | 输出 | 继电器/锁存模块 信号(IN) | 2 线（信号 + GND，无 VCC）；切换 12V 负载 |
| **GPIO9** | BOOT 按钮 | 输入 | 板载（接 GND） | `INPUT_PULLUP`；长按进 WiFi 模式 |
| **GPIO8** | 蓝色 LED | 输出 | 板载 | WiFi 模式闪烁 |
| **GPIO4** | SPI CS | 输出 | MicroSD CS | FSPI |
| **GPIO3** | SPI MOSI | 输出 | MicroSD MOSI | |
| **GPIO2** | SPI SCK | 输出 | MicroSD CLK | |
| **GPIO1** | SPI MISO | 输入 | MicroSD MISO | |
| **GPIO0** | ADC1_CH0 | 输入 | 分压抽头（R1 33k / R2 10k） | 电池电压采样 |
| **3V3** | 电源 | 输出 | DS3231、SHT35、MicroSD | 继电器/锁存无 VCC |
| **GND** | 地 | — | 所有模块 + 电池负极 | 必须共地 |

> GPIO8（蓝色 LED）和 GPIO9（BOOT 按钮）是 ESP32-C3 SuperMini 的**板载**元件，不是外接件。
...
> **实测行为 / 已知限制**：在 33k/10k 分压下，只要电池高于约 12.6V，抽头电压就超过 ESP32-C3 ADC 的可用上限
> （约 2.9V），ADC **饱和**，于是整个上半区报告的电压都被钉在约 13.42V / ~99%（电池健康时网页恒显示 ~99%）。
> 这是硬件限制，**不是固件 bug**——软件无法救回已饱和的 ADC。电池低于约 12.6V 时 ADC 重新进入可用区，因此
> **低电断电、充电恢复、重启都能正常工作**（它们都作用在低电压区）。由于固件比值 4.61 在低电区会读高约 6.5%，
> 断电实际比标称的 12.5V 更深一点（真实约 11.7~12V）——仍然安全，且有电池 BMS 兜底。若要让整个量程都可信，
> 需减小分压（如 R2 10k → 6.8k，比值约 5.85）让抽头落回 ADC 可用区。
